    ...
    $.ajax({
        ...
        success: function (response){
            response.forEach(row => {
                ...
                <td>${row['sub_total']}</td>
                <td>
                    <button class="btn btn-success" type="button" onclick="editDetalle(${row['id']})">
...
function editDetalle(id) {
    window.location.href = 'editarProductoVenta.php?id='+id+'';
}
...
const cambioCantidad = document.getElementById('cantidad_edit');
cambioCantidad.addEventListener("input", () =>{
    let cantidad = document.getElementById('cantidad_edit').value;
    let parcial = document.getElementById('precio_base').value;
    let total = cantidad * parcial;
    let valor = document.getElementById('total_edit');
    valor.value = total.toFixed(2);
    
})

const cambioTotal = document.getElementById('total_edit');
cambioTotal.addEventListener("input", () =>{
    let total = document.getElementById('total_edit').value;
    let parcial = document.getElementById('precio_base').value;
    let cantidad = total / parcial;
    let cant = document.getElementById('cantidad_edit');
    cant.value =  cantidad.toFixed(2);
